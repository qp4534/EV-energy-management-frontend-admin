...
async function updateCarFields(carId: string, fields: Partial<BackendCarDto>): Promise<void> {
...
export async function uploadVehicleImage(
  ...
  const { data } = await apiClient.post<{ uploadUrl: string; imageUrl: string }>(
    `/api/cars/${carId}/image/upload-url`,
    { contentType }
  );
...
export async function setVehicleImage(carId: string, imageUrl: string): Promise<void> {
  if (!USE_MOCK) {
